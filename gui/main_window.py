...
• Общий ритм печати"""
    ...
    def on_training_complete(self):
        """Обработка завершения первичного обучения - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
        ...
        try:
            ...
            trainer = SimpleKNNTrainer.load_model(self.current_user.id)
            if trainer and trainer.training_stats:
                print("🎉 Показываем результаты первичного обучения")
                from gui.training_visualization_window import TrainingVisualizationWindow
                TrainingVisualizationWindow(self.root, self.current_user, trainer.training_stats)
            else:
                print("⚠️ Используем базовую информацию для первичного обучения")
    ...
    def _training_completed(self, success: bool, accuracy: float, message: str, progress_window):
        """Завершение переобучения - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
        ...
        if success:
            ...
            # ✅ ИСПРАВЛЕНИЕ: Используем правильный класс для показа результатов
            try:
                from ml.simple_knn_trainer import SimpleKNNTrainer
                trainer = SimpleKNNTrainer.load_model(self.current_user.id)
                if trainer and trainer.training_stats:
                    print("✅ Показываем результаты переобучения")
                    from gui.training_visualization_window import TrainingVisualizationWindow
                    TrainingVisualizationWindow(self.root, self.current_user, trainer.training_stats)
                else:
                    print("⚠️ Нет данных для визуализации, показываем базовую информацию")
                    self._show_basic_training_info()
            except Exception as e:
    ...
    def show_training_results(self):
        """Показать результаты последнего обучения модели - ПОЛНОСТЬЮ ИСПРАВЛЕННАЯ ВЕРСИЯ"""
        if not self.current_user or not self.current_user.is_trained:
            messagebox.showwarning("Предупреждение", "Модель не обучена")
            return

        try:
            # ✅ ИСПРАВЛЕНИЕ: Используем правильный класс!
            from ml.simple_knn_trainer import SimpleKNNTrainer
            trainer = SimpleKNNTrainer.load_model(self.current_user.id)
            
            if trainer and trainer.training_stats:
                print(f"✅ Загружены результаты обучения:")
                print(f"   Точность: {trainer.training_stats.get('test_accuracy', 0):.2%}")
                print(f"   ROC AUC: {trainer.training_stats.get('roc_auc', 'нет данных')}")
                print(f"   Параметры: {trainer.training_stats.get('best_params', {})}")
                
                # ✅ Показываем окно с результатами
                from gui.training_visualization_window import TrainingVisualizationWindow
                TrainingVisualizationWindow(self.root, self.current_user, trainer.training_stats)
            else:
                # ✅ Если нет данных, показываем базовую информацию
                print("⚠️ Детальные результаты не найдены, показываем базовую информацию")
                self._show_basic_training_info()
    ...
    def _show_basic_training_info(self):
        ...
        try:
            # ✅ ИСПРАВЛЕНИЕ: Добавляем импорт datetime
            from datetime import datetime
            ...
            print("📊 Показываем базовую статистику обучения")
            ...
            print(f"❌ Ошибка создания базовой статистики: {e}")
            messagebox.showinfo("Информация", 
                ...
                f"• Дата регистрации: {self.current_user.created_at.strftime('%d.%m.%Y') if self.current_user.created_at else 'Неизвестно'}\n\n"
